...
assessmentButton.addEventListener(
  ...
  () => {
    ...
    resultDivision.innerText = '';// 初期化（id=result-areaの内容）
    const header = document.createElement('h3');// h3タグを作成
    header.innerText = '診断結果'; //h3タグの中身に書き込む
    resultDivision.appendChild(header); //id=result-areaのdivタグの子要素として追加

    const paragraph = document.createElement('p');// pタグを作成
    const result = assessment(userName);// 機能を実行
    paragraph.innerText = result;//結果をpタグの中身に書き込む
    resultDivision.appendChild(paragraph);//id=result-areaのdivタグに子要素として追加（headerの次に表示させる）

    // ツイートエリアの作成
    // while(tweetDivision.firstChild){ //練習問題
